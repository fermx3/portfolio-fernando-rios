...
impact: "Six kinds of user work in the same system, each seeing only their own data. The database enforces that separation, not the code, so a programming mistake cannot leak one company's data into another. The website and the iOS and Android apps behave the same."
